...
// Solved: Palindrome — Frequency Observation (Google keep)
/*
...
int main() {
    ...
    while(cs--) {
        ...
        if(k < 0) {
            if(k == -1 && (!n || n+1 % 2)) cout << "YES" << endl; // one odd number space
            else cout << "NO" << endl;
        }
        else if(k == 0) cout << "YES" << endl;
        else { // k > 0
            if(k % 2 && (n-k) % 2) cout << "YES" << endl;
            else if(k % 2 == 0 && (n-k) % 2 == 0) cout << "YES" << endl;
            else cout << "NO" << endl;
        }
    }
